...
public class PBoardDAO {
	...
	public List getBoardList(int startRow,int pageSize) {
		...
		sql="select * from pboard order by num desc limit ?,?";
	...
	public List getBoardList(int startRow,int pageSize, String search) {
		...
		sql="select * from pboard where subject like ? order by num desc limit ?,?";
...
//			sql="select count(*) from board where subject like '%제목%'";
			sql="select count(*) from pboard where subject like ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%"+search+"%");
	...
	public void updateReadcount(int num) {
		...
		try {
			...
			sql="update pboard set readcount= readcount+1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public PBoardBean getBoard(int num) {
		...
		try {
			...
			sql="select * from pboard where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public void  insertBoard(PBoardBean bb) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql="select max(num) as mnum from pboard";
			pstmt=con.prepareStatement(sql);
			// 4단계 실행 => 결과 저장
			rs=pstmt.executeQuery();
			...
			sql="insert into pboard(num,name,pass,subject,content,readcount,date,file) values(?,?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
		...
		try {
			...
			if(rs.next()) {
				if(bb.getPass().equals(rs.getString("pass"))) {
					//3 sql update
					sql="update pboard set name=?,subject=?,content=?, file=? where num=?";
					pstmt=con.prepareStatement(sql);
	...
	public int deleteBoard(PBoardBean bb) {
		Connection con=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String sql="";
		...
		try {
			...
			if(rs.next()) {
				if(bb.getPass().equals(rs.getString("pass"))) {
					//3 sql delete
					sql="delete from pboard where num=?";
					pstmt=con.prepareStatement(sql);
					pstmt.setInt(1, bb.getNum());
					//4 실행
					pstmt.executeUpdate();
