...
for(int i = 0; i<boardList.size(); i++){
	...
	%>
	<tr onclick="location.href='fcontent.jsp?num=<%=bb.getNum()%>'"><td><%=bb.getNum() %></td><td class="left"><%=bb.getSubject() %></td>
...
if(id!=null){
	%>
<div id="table_search">
<input type="submit" name="글쓰기" value="글쓰기" class="btn" onclick="location.href='fwriteForm.jsp'">
</div>
	<%
...
if(startPage > pageBlock){
	%><a href="fnoticeSearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">[이전]</a><%
}
		
for(int i=startPage;i<=endPage;i++){
	%><a href="fnoticeSearch.jsp?pageNum=<%=i%>"><%=i%></a> <%
}

//[다음]
if(endPage < pageCount){
	%><a href="fnoticeSearch.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search%>">[다음]</a><%
}
%>
